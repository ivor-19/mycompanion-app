...
export default function MoodCard({ moodData, handleDeleteMood }: Props) {
  ...
  return(
    <>
      <TouchableOpacity activeOpacity={0.8} className="bg-white" onPress={() => setOpen(true)}>
        <View className="bg-white py-6 flex-row items-center justify-center gap-4">                
          <View className="items-center justify-center h-8 w-8 rounded-full">
            <Image source={getEmojiByMood(moodData.moodText)} contentFit="contain" style={{height: 30, width: 30}}/>
          </View>                                        
          <View className="flex-1">
            <Text className="font-funnel_semi" style={{fontSize: FONT.xs}}>{moodData.date}</Text>
            <Text className="font-funnel_regular mb-1" style={{fontSize: FONT.xxs}} numberOfLines={2}>{moodData.note}</Text>

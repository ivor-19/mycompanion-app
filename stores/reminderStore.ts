...
const useReminderStore = create<ReminderStore>()(
  persist(
    (set, get) => ({
      ...
      initializeNotifications: async () => {
        await initializeNotificationPermissions();
        get().setupNotificationListener();
      },

      // Setup listener to auto-delete reminders when notification is received
      setupNotificationListener: () => {
        console.log('📡 Setting up notification listener for auto-delete');
        
        Notifications.addNotificationReceivedListener((notification) => {
          const reminderId = notification.request.content.data?.reminderId;
          
          if (reminderId) {
            console.log('🔔 Notification received, auto-deleting reminder:', reminderId);
            // Auto-delete the reminder after it triggers
            get().deleteReminder(reminderId as string);
          }
        });
      },
      ...
      // ADD REMINDER - Simple daily notification that auto-deletes
      addReminder: async (name, hour, minute, period, soundEnabled) => {
        console.log('🔵 addReminder called');
        ...
        console.log(`📅 Scheduling daily notification for: ${name} at ${hour24}:${min}`);
        ...
        try {
          ...
          // Update with notification ID
          get()._updateReminderInState(newReminder.id, { notificationId });
          console.log('✅ Notification ID updated');
        } catch (error) {
          console.error('❌ Error scheduling notification:', error);
        }
      ...
      deleteReminder: (id) => {
        ...
        if (reminder?.notificationId) {
          Notifications.cancelScheduledNotificationAsync(reminder.notificationId)
            .then(() => console.log('✅ Cancelled notification:', reminder.notificationId))
            .catch((error) => console.error('Error canceling notification:', error));
        }
      },
      ...
      toggleReminder: async (id) => {
        ...
        if (reminder.isEnabled) {
          // DISABLE
          console.log('⏸️ Disabling reminder:', id);
          get()._updateReminderInState(id, { isEnabled: false, notificationId: undefined });
          ...
          try {
            ...
            get()._updateReminderInState(id, { isEnabled: true, notificationId });
            console.log('✅ Reminder re-enabled');
          } catch (error) {
            console.error('Error re-enabling notification:', error);
          }
